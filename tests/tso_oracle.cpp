...
int main() {
    if (kActions.size() != 23) {
        throw std::runtime_error(
            "TSO oracle alphabet must include TimedWait, rwlocks, and condvars");
    }
    ...
    for (std::size_t i = 0; i < condition_kinds.size(); ++i) {
        ...
        if (action.kind != condition_kinds.at(i) ||
            ...
            throw std::runtime_error(
                "TSO oracle must contain valid Wait/TimedWait/Signal/Broadcast on cv");
        }
    }
    ...
    cross_validate_program(model::Program{{
                           ...
                           cycle_counts);
    cross_validate_program(model::Program{{
                               {set(7, 1), lock("m"), label("retry"),
                                unlock("m"), lock("m"), bnz(7, "retry")},
                               {lock("m")},
                           }},
                           programs_checked,
                           skipped_capped,
    ...
    // explicit TSO flush transition drains it. A transient peer reader also
    ...
    if (cycle_counts.naive_fair == 0 || cycle_counts.dpor_fair == 0 ||
        ...
        throw std::runtime_error("TSO tri-state cycle gate is vacuous");
    }
